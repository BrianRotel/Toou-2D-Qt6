...
public:
     ...
     * */
    Q_PROPERTY(QString appStartupTheme READ getAppStartupTheme WRITE setAppStartupTheme NOTIFY
                   appStartupThemeChanged)

    /*!
     * 导入皮肤的路径
     * 可以设置多个不同的路径。满足各种设计需求。可以是QRC内部，APP外部等。
     * */
    Q_PROPERTY(QStringList appThemePaths READ getAppThemePaths WRITE setAppThemePaths NOTIFY
                   appThemePathsChanged)

    /*!
    此属性全局，保存鼠标区域的光标形状。
    */
    Q_PROPERTY(int mouseAreaCursorShape READ mouseAreaCursorShape WRITE setMouseAreaCursorShape
                   NOTIFY mouseAreaCursorShapeChanged)
...
signals:
    void appStartupThemeChanged();
    void appThemePathsChanged();
    void mouseAreaCursorShapeChanged();
private slots:
