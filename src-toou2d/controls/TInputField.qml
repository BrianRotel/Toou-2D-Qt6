...
Item {
    ...
    TObject {
        id: _private
        property bool hold: input.focus || input.length > 0
    ...
    Row {
        ...
        property int leftPadding: {
            if (!iconloader.visible
                    && background.radius >= toou2d_textinput.height / 2) {
        ...
        Loader {
            ...
            sourceComponent: {
                if (!micon.source)
                    return null
                else if (micon.type === TIconType.SVG || micon.source.indexOf(
                             ".svg") != -1) {
                    return svgComponent
                }
