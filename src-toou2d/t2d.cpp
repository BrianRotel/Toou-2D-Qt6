...
void T2D::registerTypes(const char *uri)
{
    ...
    qmlRegisterType(QUrl("qrc:/net.toou.2d/controls/TMouseArea.qml"),
                    uri,
                    major,
                    minor,
                    "TMouseArea");
    qmlRegisterType(QUrl("qrc:/net.toou.2d/controls/TFlickable.qml"),
                    uri,
                    major,
                    minor,
                    "TFlickable");
...
QString T2D::awesomeFromKey(const QString &key)
{
    if(!key.isEmpty()){
        ...
        // Qt 5: 直接使用整数构造（隐式转换）
        return QString(Toou2D_Awesome::staticMetaObject.enumerator(index).keysToValue(
        ...
        return QString(QChar(static_cast<char16_t>(charCode)));
#endif
    }
...
#else
    // Qt 6: 需要显式转换
    return QString(QChar(static_cast<char16_t>(type)));
#endif
}
